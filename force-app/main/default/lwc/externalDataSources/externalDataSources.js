...
export default class ExternalDataSources extends LightningElement {
  ...
  getExternalDataSourcePermissionsCallback({ error, data }) {
    ...
    if (error) {
      ...
      this.records = [];
      this.dispatchSubtitle(["0 External Data Sources"]);
      this.dispatchEvent(
  ...
  updateSubtitle() {
    ...
    if (this.searchTerm && this.searchTerm.trim().length > 0) {
      this.dispatchSubtitle([`${filtered} of ${total} External Data Sources`]);
    } else {
      this.dispatchSubtitle([`${total} External Data Sources`]);
    }
  }
