...
export default class AppPermissions extends LightningElement {
  ...
  getAppPermissionsCallback({ error, data }) {
    ...
    if (error) {
      ...
      this.records = [];
      this.dispatchSubtitle(["0 App Permissions enabled"]);
      this.dispatchEvent(
  ...
  updateSubtitle() {
    ...
    if (this.searchTerm && this.searchTerm.trim().length > 0) {
      this.dispatchSubtitle([
        `${filtered} of ${total} App Permissions enabled`
      ]);
    } else {
      this.dispatchSubtitle([`${total} App Permissions enabled`]);
    }
  }
