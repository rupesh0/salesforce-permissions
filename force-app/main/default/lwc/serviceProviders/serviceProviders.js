...
export default class ServiceProviders extends LightningElement {
  ...
  getServiceProviderPermissionsCallback({ error, data }) {
    ...
    if (error) {
      ...
      this.records = [];
      this.dispatchSubtitle(["0 Service Providers"]);
      this.dispatchEvent(
  ...
  updateSubtitle() {
    ...
    if (this.searchTerm && this.searchTerm.trim().length > 0) {
      this.dispatchSubtitle([`${filtered} of ${total} Service Providers`]);
    } else {
      this.dispatchSubtitle([`${total} Service Providers`]);
    }
  }
  ...
  get filteredRecords() {
    ...
    return this.records.filter(
      (row) =>
        row.serviceProviderName?.toLowerCase().includes(term) ||
        row.startUrl?.toLowerCase().includes(term)
    );
  }
